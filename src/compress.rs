...
pub fn zip_compress(file: Vec<PathBuf>) -> Result<(), Box<dyn Error>> {
    for entry in &file {
        ...
        let parent_dir = &entry.parent().ok_or("no parent directory found")?;
        ...
        let opened_file = File::open(&entry)?;
        let mut buffer = BufReader::new(opened_file);
